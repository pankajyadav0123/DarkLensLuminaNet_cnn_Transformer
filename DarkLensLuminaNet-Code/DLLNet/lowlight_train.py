...
def train(config):
	...
	print(DCE_net.parameters())
	print(sum(p.numel() for p in DCE_net.parameters()))
	...
	for epoch in range(config.num_epochs):
		for iteration, img_lowlight in enumerate(train_loader):
			...
			psnrr = psnr(img_lowlight, enhanced_image)
			print(f"PSNR: {psnrr:.2f}")
			sum_psnrr = sum_psnrr + psnrr
		...
		#print(optimizer)
	avg_psnr = sum_psnrr / 250.0
	print("AVG_psnr : ",avg_psnr,sum_psnrr)
